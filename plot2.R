# plot2.R -> Global Active Power by combined granula DateTime column
...
# For this plot, need to add day of the week; i.e., it appears in the X-axis of the demo PNG
# I thought that adding the Day of Week would be enough...but while the following does that
#   subsequently plotting against a "Day of week" column doesn't replicate the demo PNG; doh!
# Feb2days <- Feb2days %>% mutate(Day_of_week = as.Date(Feb2days$Date, format = "%a"))

# But no, instead it's about a very granular DateTime object...so add a new column that merges Date w/Time
Feb2days <- Feb2days %>% mutate(Date_Time = as.POSIXct(paste(Feb2days$Date, Feb2days$Time), format="%Y-%m-%d %H:%M:%S"))

# 4. Create the plot; see spec above / demo image online; note it has no X-axis label, unlike plot1.png

png("plot2.png", width=480, height=480)
with(Feb2days, plot(Date_Time, Global_active_power, type="l", xlab = "", ylab="Global Active Power (kilowatts)"))
dev.off()
